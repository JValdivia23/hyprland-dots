...
cp -v "$SCRIPT_DIR/blacklist-touchbar.conf" /etc/modprobe.d/blacklist-touchbar.conf

# 2. Install udev rules
echo "[2/5] Installing udev rules for Touch Bar USB Configuration 2..."
cp -v "$SCRIPT_DIR/99-touchbar-tiny-dfr.rules" /etc/udev/rules.d/99-touchbar-tiny-dfr.rules
udevadm control --reload-rules

# 3. Install sleep helper
echo "[3/5] Installing updated /usr/local/bin/t2-sleep-helper..."
cp -v "$SCRIPT_DIR/t2-sleep-helper" /usr/local/bin/t2-sleep-helper
chmod +x /usr/local/bin/t2-sleep-helper

# 4. Install systemd services
echo "[4/5] Installing and enabling systemd units..."
cp -v "$SCRIPT_DIR/tiny-dfr.service" /etc/systemd/system/tiny-dfr.service
cp -v "$SCRIPT_DIR/t2-touchbar-setup.service" /etc/systemd/system/t2-touchbar-setup.service
...
sleep 2
systemctl status tiny-dfr.service --no-pager || true
